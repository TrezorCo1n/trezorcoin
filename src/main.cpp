...
#if defined(NDEBUG)
# error "Trezorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x63013e2fcde7747a2598f4ad1b89dea14eb3177ce4410262a28593e559d13857");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Trezorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Trezorcoin Signed Message:\n";
...
{
    // Trezorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Trezorcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Trezorcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Trezorcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Trezorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Trezorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x096503e2767d321490b76a33470a1bd500f9fce8f996b2090b9e65aab93f4cd1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 05/Oct/2021 CO-VID 19, Corona virus Pandemic, starts in 2019";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e6b1fd47fc326571ea8f26808e1a76156f990dd6a473850e72f1117fe5034f436c0dc86c071020adcdc620f4b823171af41a89fb254c1ad63ffc8b2e5c215ac8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1637785362;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087519226;

        if (fTestNet)
        {
            block.nTime    = 1637785330;
            block.nNonce   = 385429299;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa331fa7b964c064e2ad8eae8baec3849beda2ac40629e3c42477bfe537079075"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc2, 0xb7, 0xd2 }; // Trezorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TrezorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TrezorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TrezorcoinMiner(CWallet *pwallet)
{
    printf("TrezorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("trezorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TrezorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TrezorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TrezorcoinMiner, pwallet));
}
